...
function devServer(options) {
	...
	const rewriteRequestForBase = require_utils.createBasePathRewriter(options?.base ?? defaultOptions.base);
	const shouldHandlePath = require_utils.createBasePathGuard(options?.base ?? defaultOptions.base);
	...
	return {
		...
		config: (config) => {
			...
			return {
				...baseOption !== void 0 ? { base: baseOption === "" ? "/" : baseOption } : {},
				server: { watch: { ignored: options?.ignoreWatching ?? defaultOptions.ignoreWatching } }
			};
		},
		configResolved(config) {
			...
			if (viteBaseSetByUser !== void 0 && viteBaseSetByUser !== "/" && honoBase !== void 0) console.warn(`The following Vite config options will be overridden by @hono/vite-dev-server:
		...
		configureServer: async (server) => {
			async function createMiddleware(server) {
				return async function(req, res, next) {
					if (req.url) {
						const filePath = path.default.join(publicDirPath, req.url);
						try {
							if (fs.default.existsSync(filePath) && fs.default.statSync(filePath).isFile()) return next();
						} catch {}
					}
					const exclude = options?.exclude ?? defaultOptions.exclude;
					for (const pattern of exclude) if (req.url) {
						if (pattern instanceof RegExp) {
							if (pattern.test(req.url)) return next();
						} else if ((0, minimatch.minimatch)(req.url?.toString(), pattern)) return next();
					}
					if (!shouldHandlePath(req.url ? require_utils.safeParseUrlPath(req.url) : void 0)) return next();
					let loadModule;
					if (options?.loadModule) loadModule = options.loadModule;
					else loadModule = async (server, entry) => {
						let appModule;
						try {
						...
						if (!app) throw new Error(`Failed to find a named export "${exportName}" from ${entry}`);
						return app;
					};
					let app;
					try {
					...
					(0, _hono_node_server.getRequestListener)(async (request) => {
						let env = {
							incoming: req,
							outgoing: res,
							vite: server
						};
						if (options?.env) if (typeof options.env === "function") env = {
							...env,
							...await options.env()
						};
						else env = {
						...
						*/
						...
						if (options?.injectClientScript !== false && response.headers.get("content-type")?.match(/^text\/html/)) {
							const viteScript = path.default.posix.join(viteBase, "/@vite/client");
							const nonce = response.headers.get("content-security-policy")?.match(/'nonce-([^']+)'/)?.[1];
							return injectStringToResponse(response, `<script${nonce ? ` nonce="${nonce}"` : ""}>import("${viteScript}")<\/script>`) ?? response;
						}
						return response;
					}, {
						...
						errorHandler: (e) => {
							...
							} else if (typeof e === "string") err = /* @__PURE__ */ new Error(`The response is not an instance of "Response", but: ${e}`);
							else err = /* @__PURE__ */ new Error(`Unknown error: ${e}`);
							next(err);
						}
					})(req, res);
				};
...
function injectStringToResponse(response, content) {
	...
	const combinedStream = new ReadableStream({ async start(controller) {
		for (;;) {
