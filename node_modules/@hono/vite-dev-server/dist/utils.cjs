...
const normalizeBasePath = (base) => {
	if (!base || base === "/") return "";
	return (base.startsWith("/") ? base : `/${base}`).replace(/\/+/g, "/").replace(/\/+$/g, "");
};
/**
...
const createBasePathRewriter = (base) => {
	...
	return (request) => {
		...
		const { pathname } = url;
		if (pathname === normalizedBase) url.pathname = "/";
		else if (pathname.startsWith(prefixWithSlash)) url.pathname = pathname.slice(prefixLength) || "/";
		else return request;
...
//#endregion
exports.createBasePathGuard = createBasePathGuard;
exports.createBasePathRewriter = createBasePathRewriter;
exports.normalizeBasePath = normalizeBasePath;
exports.safeParseUrlPath = safeParseUrlPath;
